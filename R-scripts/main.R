...
de_markers_condition <- FindAllMarkers(integrated, only.pos = T) # for gene set ENRICHMENT analysis
filtered_markers <- de_markers_condition %>% filter(p_val_adj < 0.05)
# write.csv(filtered_markers, "240614_filtered(p)_markers.csv", row.names = FALSE)
filtered_markers <- read.csv(file = '240614_filtered(p)_markers.csv')

top10 <- filtered_markers %>% 
  group_by(cluster) %>% 
  top_n(10, avg_log2FC) 

# write.csv(top10, "240614_filtered(p)_markers_top10.csv", row.names = FALSE)
...
write.csv(top20, "240614_filtered(p)_markers_top20.csv", row.names = FALSE)
DoHeatmap(integrated, features = top20$gene) + theme(axis.text.y = element_text(size = 6))

# read csv 
DE_marker <-  read.csv(file = "240614_filtered(p)_markers.csv")
top10 <- read.csv(file = '240614_filtered(p)_markers_top10.csv')
top20 <- read.csv(file = '240614_filtered(p)_markers_top20.csv')
...
DotPlot(object = integrated, features = c('SOST', 'REN', 'AC239799.2', 'Z93241.1', 'AC012447.1', 'CCN4', 'GAL', 'ADAMTS18', 'AL021155.5', 'VSTM1'), group.by = "Condition") +
  ...
  xlab("Condition") +
  ylab("Features")
dev.copy(png, file = "240630-top10-expression-prestim-dotplot-1059x705.png", width = 900, height = 700)
dev.off()  # Close the PNG device
...
DotPlot(object = integrated, features = c('AL355075.4','AP000692.2','TEX14','ADAMTS20','NXPH2','CD3D','LINC01629','CKS1B','LIN28A','DMKN'), group.by = "Condition") +
  ...
  xlab("Condition") +
  ylab("Features")
dev.copy(png, file = "240630-top20-expression-prestim-dotplot-1059x705.png", width = 900, height = 700)
dev.off()  # Close the PNG device
...
dev.copy(png, file = "240630-top10-expression-static-1059x705.png", width = 1059, height = 705)
dev.off()  # Close the PNG device
...
dev.copy(png, file = "240630-top10-expression-static-dotplot-1059x705.png", width = 900, height = 700)
dev.off()  # Close the PNG device

# Top20: C1QB, S100P
FeaturePlot(object = integrated, features = c('P2RY13','IL1RN','CD14','RETN','FGR','C1QB','MCEMP1','S100P','HLA-DRB5','AC016168.4'))
dev.copy(png, file = "240630-top20-expression-static-1059x705.png", width = 1059, height = 705)
dev.off()
...
dev.copy(png, file = "240630-top20-expression-static-dotplot-1059x705.png", width = 900, height = 700)
dev.off()  # Close the PNG device
...
dev.copy(png, file = "240630-top10-expression-laminar-1059x705.png", width = 1059, height = 705)
dev.off()  # Close the PNG device
...
dev.copy(png, file = "240630-top10-expression-laminar-dotplot-900x700.png", width = 900, height = 700)
dev.off()  # Close the PNG device
...
dev.copy(png, file = "240630-top20-expression-laminar-1059x705.png", width = 1059, height = 705)
dev.off() 
...
dev.copy(png, file = "240630-top20-expression-laminar-dotplot-900x700.png", width = 900, height = 700)
dev.off()  # Close the PNG device
...
dev.copy(png, file = "240630-TRANSMEMBRANE-TRANSPORTER-EXPRESSION-1059x705.png", width = 1059, height = 705)
dev.off() 
...
dev.copy(png, file = "240630-TRANSMEMBRANE-TRANSPORTER-EXPRESSION-dotplot-900x700.png", width = 900, height = 700)
dev.off()  # Close the PNG device
